...
class DefaultSNNETTrainer(TrainerBase):
    """
    A trainer that uses a SNNET backbone with default training logic. It does the following:
    ...
    def __init__(self, cfg):
        ...
        if not logger.isEnabledFor(logging.INFO):  # setup_logger is not called for d2
            setup_logger()
        cfg = DefaultSNNETTrainer.auto_scale_workers(cfg, comm.get_world_size())
    ...
    def resume_or_load(self, resume=True):
        ...
        if self.cfg.MODEL.WEIGHTS:
            ...
            for i, anchor_name in enumerate(["TINY", "SMALL", "BASE"]):
                ...
                temp_checkpointer.load(self.cfg.SWIN_SNNET[anchor_name].WEIGHTS)
...
class DefaultWSTrainer(TrainerBase):
    """
    A WS trainer with default training logic. It does the following:
    ...
    def __init__(self, cfg):
        ...
        if not logger.isEnabledFor(logging.INFO):  # setup_logger is not called for d2
            setup_logger()
        cfg = DefaultWSTrainer.auto_scale_workers(cfg, comm.get_world_size())
